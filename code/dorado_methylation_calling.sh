...
show_help() {
  cat <<EOM
Usage: $0 -p <port> -d <cuda_devices> -t <threads> file.blow5
...
[ -z ${SLOW5_DORADO} ] && export SLOW5_DORADO=slow5-dorado
${SLOW5_DORADO} --version &> /dev/null || die "$(cmd_not_found slow5-dorado)"
...
${SLOW5_DORADO} basecaller -x cuda:${devices} $model  $1  --modified-bases "5mCG_5hmCG" > $name.sam \
